    ...
    $(document).ready(function() {
        ...
        */
        ...
        for (let itemId in cart) {
            ...
            //get ajax request to get the seller name
            $.ajax({
                url: '/product/get-seller/' + item.user_id,
                type: 'get',
                async: true,
                success: function(response) {
                    console.log(response.seller.name)
                    $('#seller-product-id-' + item.id).html(response.seller.name);
                }
            });
                ...
                <td class="col-sm-8 col-md-6">
                    <div class="media">
                        ...
                        <div class="media-body">
                            ...
                            <h5 class="media-heading">Vendido por <span id="seller-product-id-${item.id}">${item.user_id}</span></h5>
        ...
        <tr>
            ...
            <td>   </td>
            <td>
                <form id="payForm" action="{{ url('charge') }}" method="post">
        ...
        $('#payBtn').click(function(e) {
            if (totalAmount <= 0 || $.isEmptyObject(cart)) {
                e.preventDefault();
            }
        })
        ...
        $('#confirm_delete_cart_btn').click(function() {
            ...
            // Update the total amount and discount
            let totalAmount = 0;
            for (let itemId in cart) {
                let item = cart[itemId];
                totalAmount += item.price * item.quantity;
            ...
            // Update the final amount and the pay button
            let finalAmount = totalAmount - discount;
            $("input[name='amount']").val(finalAmount < 0 ? 0 : finalAmount);
            $("#payBtn").val('Pagar ' + finalAmount + ' €');

            // Update the actual text displayed on the page
            $('h3:contains("€")').text(`€${totalAmount}`);
            $('strong:contains("puntos")').text(`${discount} puntos`);
