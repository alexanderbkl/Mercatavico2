        ...
        @if ($userProducts && count($userProducts) > 0)
            ...
            @foreach ($userProducts as $producto)
                <tr class="product_item_list" data-product_id="{{ $producto->id }}" data-title="{{ $producto->title }}"
                    data-description="{{ $producto->description }}" data-estado="{{ $producto->status }}">
                    <th scope="row">
                    ...
                    <td>
                        ...
                        @endphp
                        <button class="btn btn-success edit_product" data-toggle="modal"
                            data-target="#modalEditProducts" data-foto="{{ $producto->foto }}"
                            data-id="{{ $producto->id }}" data-materiales="{{ $arrayMateriales }}"
                            data-title="{{ $producto->title }}" data-description="{{ $producto->description }}"
                            data-price="{{ $producto->price }}" data-stock="{{ $producto->stock }}"
                            data-status="{{ $producto->status }}">Editar</button>
                        <button class="btn btn-danger deleteProduct" data-id="{{ $producto->id }}" data-toggle="modal"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
                ...
                <form id="editProduct">
                    <div class="row" style="width: 100%">
                        ...
                        <div class="form-group col-12" style="text-align: center">
                            ...
                            <input id="foto_edit_input" type="file" name="foto_edit" accept="image/*"
                                class="text-center center-block foto_edit">
                        </div>
                        ...
                        <div class="form-group col-12 col-md-6">
                            <div class="col-xs-6">
                                ...
                                <select class="form-control" id="inputSelectMaterialesEdit" name="materiales"
                                    multiple="multiple">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="btnUpdateProduct" data-dismiss="modal">Guardar
                    producto</button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="confirmDeleteModal" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
                <button type="button" class="btn btn-primary confirm_delete_btn"
...
<script>
    ...
    $('.edit_product').click((e) => {
        $('.img-thumbnail_edit').attr('src', '{{ asset('storage/productsImages/') }}/' + e.currentTarget
            .dataset.foto)
